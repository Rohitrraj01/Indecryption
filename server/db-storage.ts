...
export class SQLiteStorage implements IStorage {
  ...
  async createUser(insertUser: InsertUser): Promise<User> {
    ...
    stmt.run(id, insertUser.username, insertUser.mobileNumber, insertUser.publicKey, false, createdAt.toISOString());
  ...
  async getContactsByUserId(userId: string): Promise<Contact[]> {
    const stmt = db.prepare(
      "SELECT * FROM contacts WHERE userId = ? ORDER BY createdAt DESC"
    );
  ...
  async getContactWithUser(userId: string, contactUserId: string): Promise<(Contact & { user: User }) | undefined> {
    const stmt = db.prepare(
      "SELECT c.*, u.* FROM contacts c LEFT JOIN users u ON c.contactUserId = u.id WHERE c.userId = ? AND c.contactUserId = ?"
    );
    const row = stmt.get(userId, contactUserId) as any;
    
    if (!row) return undefined;
    
    const contact = this.mapToContact(row);
    const user = row.id && row.username ? this.mapToUser(row) : null;
    
    return user ? { ...contact, user } : undefined;
  }
